...
public class GameChoices extends GamePlayer
{
	//solo slim  13slagen
	//solo   13 slagen
	//openmisere  
	...
	public void setPoints(){
		...
		if(maxChoice==MISERE || maxChoice==OPENMISERE){//ok
			...
			//verdeel de punten
			int points;
			if(maxChoice==MISERE){points=15;}else{points=30;}
			switch(numberOf){
				case 1:
		...
		if(playerPoints[1]+playerPoints[2]+playerPoints[3]+playerPoints[4]!=0){
			toast("totaal puntenaantal klopt niet, fout in programma");
		}	
	}
	...
	public String getChoice(){
		...
		switch(playerChoice[player]){
			case SOLOSLIM:str="Soloslim ("+String.valueOf(playerTrumpCategory[player])+")";break;
			case SOLO:str="Solo ("+String.valueOf(playerTrumpCategory[player])+")";break;
			case OPENMISERE:str="Openmisere";break;
			case TROEL:str="Troel ("+String.valueOf(playerTrumpCategory[player])+")";break;
			case TROELMEE:str="Troelmee ("+String.valueOf(playerTrumpCategory[player])+")";break;
			case DANSEN12INTROEF:str="Dansen12introef ("+String.valueOf(playerTrumpCategory[player])+")";break;
			case DANSEN12:str="Dansen12 ("+String.valueOf(playerTrumpCategory[player])+")";break;
			case DANSEN11INTROEF:str="Dansen11introef ("+String.valueOf(playerTrumpCategory[player])+")";break;
			case DANSEN11:str="Dansen11 ("+String.valueOf(playerTrumpCategory[player])+")";break;
			case DANSEN10INTROEF:str="Dansen10introef ("+String.valueOf(playerTrumpCategory[player])+")";break;
			case DANSEN10:str="Dansen10 ("+String.valueOf(playerTrumpCategory[player])+")";break;
			case MISERE:str="Misere";break;
			case DANSEN9INTROEF:str="Dansen9introef ("+String.valueOf(playerTrumpCategory[player])+")";break;
			case DANSEN9:str="Dansen9 ("+String.valueOf(playerTrumpCategory[player])+")";break;
			case VRAAG:str="Vraag";break;
			case MEEGAAN:str="Meegaan";break;
			case ALLEEN:str="Alleen";break;
			case PAS:str="Pas("+String.valueOf(playerTrumpCategory[player])+")";break;
	...
	public void startChoice(){
		...
		//toast("choice[" + String.valueOf(player) + "] = "+ String.valueOf(choice));
	}	
	...
	private void setStringsInList2(){
		...
		switch(choice){
			...
			case SOLO:
				...
				break;
			case OPENMISERE:
				//als al iemand openmisere doet kan je het risico nemen als het vrij lage kaarten zijn
				//dus twijfelgeval dan ok,misschien??????
	...
	private boolean choiceOk(){
		...
		if(arraylist2.isEmpty()){return false;}//toast("arraylist2 empty!!!!!");
		if (ok) return true;
				...
				if (ok) {
					...
					if(choice==DANSEN9 || choice==DANSEN9INTROEF || choice==DANSEN10 || choice==DANSEN10INTROEF ||
					   choice==DANSEN11 || choice==DANSEN11INTROEF || choice==DANSEN12 || choice==DANSEN12INTROEF ||
						...
						//toast(String.valueOf(playerTrumpCategory[1]));//test
					}
					return ok;
				}
			}
	...
	private String getCardsString(int player){
		...
		int oldCat=0;
		for(int b=1;b<=13;b++){
			while(playerDeck[player].get(b-1).getCategory()!=cat){cat++;}
			if(oldCat!=cat){
				if(str.compareTo("")!=0) str=str+",";
				if(playerDeck[player].get(b-1).getCategory()==getTrumpCategory()) str=str+"!";
				oldCat=cat;
			}
			str=str+playerDeck[player].get(b-1).getStringValue();	
		}	
		return str;
	}
	...
	private void setTrumpCategory(String str){
		...
		for(int i=st2.length-1;i>-1;i--){//van rechts controleren	
			if(str.compareTo(st2[i])!=0){
				...
				//toast("pos="+pos+"  trump="+ deck.get(pos).getCategory());
				break;
			}
		}
		playerTrumpCategory[player]=deck.get(pos).getCategory();
	}
	...
	public boolean startZoekenNaar(int keuze){
		//hier gaan we waitingDeck[0] eventjes in gebruik nemen
		for(int a=1;a<=4;a++){
			waitingDeck[0].addAll(playerDeck[a]);
		}
		liftDeck(waitingDeck[0]);
		for(int a=1;a<=4;a++){playerDeck[a].clear();}
		int z=0,i=0;
		while(i<waitingDeck[0].size()){//<=51 
			if(i>30){z=5;}else{z=4;}
			for(int a=1;a<=4;a++){
				for(int b=1;b<=z;b++){
					if(i<waitingDeck[0].size()){playerDeck[a].add(waitingDeck[0].get(i));}
					i++;
				}
			}	
		}
		...
		restartChoice();
		checkTroel();
		for(int a=1;a<=4;a++){//4x
			startChoice();
			if(playerChoice[player]==keuze){return true;}//meer dan bvb 1 miserespeler vinden duurt heel lang
			setPlayer();
		}	
		return false;
	}
...
//		Toast.makeText(mCtx,pp,Toast.LENGTH_SHORT).show();
//	}
}
