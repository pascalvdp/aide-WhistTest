...
public class Game extends GameChoices
{
	...
	public void checkGameEnd(){//enkel bij misere en openmisere
	...
	private void pasStart(){
		if(maxChoice==DANSEN9 || maxChoice==DANSEN9INTROEF || maxChoice==DANSEN10 || maxChoice==DANSEN10INTROEF ||
		   maxChoice==DANSEN11 || maxChoice==DANSEN11INTROEF || maxChoice==DANSEN12 || maxChoice==DANSEN12INTROEF ||
			maxChoice>OPENMISERE){//dansen solo soloslim
			vraagStart();
		}
		...
		if(maxChoice==ALLEEN){
			vraagStart(); //alleenStart(); 25/6/2016
		}
	}
	...
	private void checkFollowed(){//enkel bij volgen(enkel bij de miserespelers)
	...
	public void setOkayAfterPlayingCard(){
		...
		if(getNumberOfPlayersMisereOrOpenmisere()==1 && maxChoice==OPENMISERE && 
			...
			int categ=gameDeck.get(gameDeck.size()-1).category;//laatste kaart in gamedeck
			//int enem=enemy[player][0];//vijand
	...
	private void vraagFollow(){
		...
		if(turn==2 || turn==3){
			if(enemyHighestValue()){
				...
				if(!hasHighestValue(getWinnerValue(),cat) && !enemyCanBuy(cat)){//24/07  !enemyCanBuy &&
	...
	private int getHighestValue(int category){
		for(card=playerDeck[player].size()-1;card>=0;card--){
	...
	private boolean safely(int player, int category){//ok ook voor openmisere
		...
		for(int a=0;a<z;a++){
			deck[a]=new ArrayList<Card>();
		}
		int d=0;
		for(int a=1;a<=4;a++){
			if(a!=player){
				if(playerChoice[a]==OPENMISERE){
					d++;deck[d].addAll(getPlayerDeckCategory(a,category));
				}else{
					deck[0].addAll(getPlayerDeckCategory(a,category));
				}	
			}
		}	
		sortingDeck(deck[0],true);
		ArrayList<Card> selfDeck = new ArrayList<Card>();
		...
		if(playerChoice[player]!=PAS && round==0 && startPlayer==player){
			for(int a=0;a<selfDeck.size();a++){
				if(selfDeck.get(a).value!=a+2){return false;}
			}	
		}else{
			for(int c=0;c<z;c++){
				for(int a=0;a<selfDeck.size();a++){
					for(int b=0;b<deck[c].size();b++){
						if(a==b && selfDeck.get(a).value>deck[c].get(b).value){return false;}
						if(a==b){break;}
					}
				}	
			}
